...
Rainfall baseline checks ignore the period-start date only for weekday and
odd/even-date cycles, where that field does not determine cadence. Interval
program dates remain protected, as do every program's starts, durations, names
and other settings. Existing legacy fingerprints require an exact match or an
explicit review in Rainfall options; they are never silently re-baselined. The
review can retain the confirmed normal budget while the current device budget
matches the integration's last saved discount.

Rainfall options retain hourly checking by default and offer a daily pre-start
mode. That mode adjusts only each selected program due to run, once per local
run date, on the first poll in the 15-minute window before its earliest enabled
start. It follows the program's watering days (interval-date predictions retain
the documented uncertainty). Checks stop two minutes before the start to leave
time for BLE writes. Failed checks may retry within that window; missed checks
are never caught up at later start slots or after watering starts. Completion
dates persist through reload/restart. Starts shortly after midnight are checked
on the preceding day. HA only updates saved settings; it never starts watering.
If HA or the rainfall sensor is unavailable, the last saved settings remain.
