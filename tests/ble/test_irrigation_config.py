...
from custom_components.solem_blip.ble import protocol
...
def test_every_write_field_including_interval_date_is_verified():
    from datetime import date
    expected=protocol.assemble_irrigation_programs(_program_a_fragments(), max_stations=12)[0]
    changed={**expected, 'name':'Changed','inter_station_delay':55,'water_budget':50,
             'cycle':4,'week_days':1,'period_length':3,'synchro_day':1,
             'period_start_date':date(2026,6,1),'start_times':[1]*8,'station_durations':[1]*12}
    assert set(protocol.irrigation_program_write_mismatches(changed,expected))==set(expected)
    assert 'program' in protocol.irrigation_program_write_mismatches(None,expected)


def test_program_packet_limits_reject_truncation_or_overflow():
    import pytest
    program=protocol.assemble_irrigation_programs(_program_a_fragments(), max_stations=12)[0]
    for patch in ({'inter_station_delay':65536},{'cycle':256},{'start_times':[0]*9},
                  {'start_times':[-1]},{'station_durations':[-1]}, {'station_durations':[0]*13}):
        with pytest.raises(ValueError):
            protocol.pack_set_irrigation_program(0,{**program,**patch},max_stations=12)
    with pytest.raises(ValueError): protocol.pack_set_irrigation_program(3,program,max_stations=12)
    with pytest.raises(ValueError): protocol.pack_set_irrigation_program(0,program,max_stations=0)
