...
from custom_components.solem_blip.ble.client_v2 import (
...
async def test_connect_passes_single_attempt_to_establish_connection(
    ...
    monkeypatch.setattr(
        "custom_components.solem_blip.ble.client_v2.establish_connection",
    ...
    assert len(calls) == 1
    args, kwargs = calls[0]
    assert issubclass(args[0], BleakClientWithServiceCache)
...
def test_request_retry_delay_is_eight_seconds() -> None:
    ...
    from custom_components.solem_blip.ble import client_v2, const
...
async def test_retry_attempts_are_spaced_by_request_retry_delay(
    ...
    monkeypatch.setattr("custom_components.solem_blip.ble.client_v2.NOTIFY_SETTLE_DELAY", 0)
    monkeypatch.setattr("custom_components.solem_blip.ble.client_v2.OPERATION_DEADLINE", 30.0)
...
async def test_status_roundtrip_single_connect(established) -> None:
    ...
    monkey_delay.setattr("custom_components.solem_blip.ble.client_v2.NOTIFY_SETTLE_DELAY", 0)
    status = await client.get_status()
    monkey_delay.undo()
...
async def test_no_state_between_operations(established) -> None:
    ...
    pytest.MonkeyPatch().setattr(
        "custom_components.solem_blip.ble.client_v2.NOTIFY_SETTLE_DELAY", 0
    )
...
async def test_deadline_bounds_repeated_connect_hangs(monkeypatch) -> None:
    ...
    monkeypatch.setattr(StatelessSolemClient, "_resolve_ble_device", fake_resolve)
    monkeypatch.setattr(StatelessSolemClient, "_connect", fake_connect)
    monkeypatch.setattr("custom_components.solem_blip.ble.client_v2.OPERATION_DEADLINE", 0.3)
    monkeypatch.setattr("custom_components.solem_blip.ble.client_v2.REQUEST_RETRY_DELAY", 0)
...
async def test_connect_phase_timeout_carries_reason_in_log_and_chain(
    ...
    client = StatelessSolemClient("AA:BB:CC:DD:EE:FF")
    with caplog.at_level(_logging.DEBUG, logger="custom_components.solem_blip.ble.client_v2"):
...
async def test_connect_timed_out_is_caught_by_same_except_tuple(monkeypatch) -> None:
    ...
    monkeypatch.setattr(asyncio, "sleep", recording_sleep)
    monkeypatch.setattr("custom_components.solem_blip.ble.client_v2.OPERATION_DEADLINE", 60.0)
...
async def test_transient_error_retries_then_succeeds(monkeypatch) -> None:
    ...
    monkeypatch.setattr("custom_components.solem_blip.ble.client_v2.NOTIFY_SETTLE_DELAY", 0)
    monkeypatch.setattr("custom_components.solem_blip.ble.client_v2.REQUEST_RETRY_DELAY", 0)
    monkeypatch.setattr("custom_components.solem_blip.ble.client_v2.OPERATION_DEADLINE", 10.0)
...
async def test_confirmed_drop_kills_attempt_then_recovers(monkeypatch) -> None:
    ...
    monkeypatch.setattr(StatelessSolemClient, "_connect", fake_connect)
    monkeypatch.setattr("custom_components.solem_blip.ble.client_v2.NOTIFY_SETTLE_DELAY", 0)
    monkeypatch.setattr("custom_components.solem_blip.ble.client_v2.OPERATION_DEADLINE", 20.0)
...
async def test_teardown_hint_does_not_poison_next_operation(
    ...
    assert client._link_dropped is False  # teardown cleared the hint
