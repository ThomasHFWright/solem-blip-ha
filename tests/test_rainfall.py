...
@pytest.mark.parametrize('cycle', [0, 1, 2, 3])
async def test_unused_date_rollover_does_not_block_baseline_restore(rainfall, cycle):
    from datetime import date
    c = rainfall.coordinator
    before = c.program_manager.snapshot
    _, normal = before.patch(0, {'cycle':cycle, 'period_start_date':date(2026,9,17)},6)
    rainfall.options['fingerprints']['0'] = program_fingerprint(dict(normal.programs[0]))
    _, discounted = normal.patch(0, {'water_budget':95,'period_start_date':date(2026,9,18)},6)
    c.api._mock_snapshot = discounted
    rainfall.state['expected_budgets'] = {'0':95}
    c.hass.states.async_set('sensor.rain','0',{'unit_of_measurement':'mm'})
    await rainfall.apply()
    assert c.irrigation_programs[0]['water_budget'] == 100
    assert c.irrigation_programs[0]['period_start_date'] == date(2026,9,18)


def test_periodic_dates_remain_protected_and_legacy_requires_exact_match():
    from datetime import date
    import hashlib
    import json
    from custom_components.solem_blip.rainfall import matches_program
    p = {'cycle':4,'period_start_date':date(2026,9,17),'water_budget':100}
    fingerprint = program_fingerprint(p)
    assert not matches_program({**p,'period_start_date':date(2026,9,18)}, fingerprint)
    p['cycle'] = 0
    legacy = hashlib.sha256(json.dumps({k:v for k,v in p.items() if k != 'water_budget'},sort_keys=True,default=str).encode()).hexdigest()
    assert matches_program(p, legacy)
    assert not matches_program({**p,'period_start_date':date(2026,9,18)},legacy)
    assert not matches_program(p,None)
