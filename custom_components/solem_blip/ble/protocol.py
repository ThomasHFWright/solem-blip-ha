...
upstream BL-IP captures; see NOTICE.md for provenance.
"""
...
def parse_controller_name_response(data: bytes | bytearray) -> str | None:
    ...
    zero-padded GATT packets too, but never infer a name from an incomplete record or
    unrelated notifications. NUL terminates the string; printable trailing
    characters are part of the name and must not be guessed away.
    """
    if (
        not 17 <= len(data) <= 20
    ...
    if len(data) < end or any(data[end:]):
        return None
    try:
        name = bytes(data[3:end]).split(b"\0", 1)[0].decode("utf-8").strip()
...
def irrigation_program_write_mismatches(
    ...
    if written["period_start_date"] != expected["period_start_date"]:
        mismatches["period_start_date"] = (expected["period_start_date"], written["period_start_date"])
...
def irrigation_config_complete(
    ...
    *,
    ...
    normalized = [frame for raw in payloads if (frame := normalize_config_notification(raw)) is not None]
    sequence_ids = {frame[2] for frame in normalized}
    # Extended V5 responses count down across all twelve storage slots. Do
    # not stop after A/B/C when later slots have not arrived (or have gaps).
    if sequence_ids and max(sequence_ids) >= 21 and sequence_ids != set(range(max(sequence_ids) + 1)):
        return False
    return all(
