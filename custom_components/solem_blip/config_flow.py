...
class SolemOptionsFlowHandler(OptionsFlowWithReload):
    ...
    async def async_step_rainfall(self, user_input: dict[str, Any] | None = None) -> ConfigFlowResult:
            ...
            try:
                ...
                if selected:
                    ...
                    for key in selected:
                        ...
                        # Submitting this form explicitly reviews the fresh program.
                        # Preserve an existing normal baseline if the current budget is
                        # our last known discount; never adopt that discount as normal.
                        confirmed = (
        ...
        fields: dict[Any, Any] = {
            ...
            vol.Required("timing", default=current.get("timing", "hourly")): selector({"select": {"options": [
                {"value": "hourly", "label": "Hourly"},
                {"value": "before_program", "label": "Daily, 15 minutes before each program's first start"},
            ]}}),
